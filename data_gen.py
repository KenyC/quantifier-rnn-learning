...
# _labeled_data = num data points sequence of pairs of sequences / labels
# sequences are the concatenation of an array of one-hot character representations
# and an array of one-hot quantifier representations
...
class DataGenerator(object):
	...
	def _tuple_to_idx(self, tup):
		...
		"""
		#No need to bother since the index are just a subset of what is needed
		seq, quant = tup
	...
	def _points_from_tuples(self, tups):
		
		input_qs = [[]]*self._num_quants
		padded_input_qs = [[]]*self._num_quants

		for char_seq, quant_idx in tups:
			# Generate char array for inputting Quantifier
			chars = tuple(quantifiers.Quantifier.chars[idx] for idx in char_seq)
			input_qs[quant_idx].append(chars)

			# Creating Padded sequences for use by NNs
			padded_seq = (chars +
						  (quantifiers.Quantifier.zero_char,)
						  * (self._max_len - len(chars)))
			padded_with_quant = tuple(
			...
			padded_input_qs[quant_idx].append(padded_with_quant)
		
		# Using multiple evaluation to get labels
		output_qs = [self._quantifiers[quant_idx].evaluateMultiple(chars) for quant_idx,chars in enumerate(input_qs)]

		return padded_input_qs, output_qs
		...
		if total_possible <= num_data_points:
			...
			while to_generate > 0:
				...
				if tup_idx not in generated_idxs:
					...
					to_generate -= 1
					list_input_tups.append(tup)

			seqs, labels = self._points_from_tuples(list_input_tups)
			if balanced:
				for i in range(self._num_quants):
					for j in range(len(seqs[i])):
						tups[(i, labels[i][j])].append(seqs[i][j])
			else:
...
if __name__ == '__main__':
	eparams = {'num_epochs': 4, 'batch_size': 8,
		   'generator_mode': 'g', 'num_data': 3000,
		   'eval_steps': 50, 'max_generation': 30}
